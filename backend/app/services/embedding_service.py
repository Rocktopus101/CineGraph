...
class EmbeddingService:
        ...
        try:
            ...
            if is_rate_limit_error(exc):
                if fallback_on_rate_limit:
                    logger.warning(
                        "%s embed rate limited during import; using zero vector",
                        self.provider.name,
                    )
                    return _zero_embedding(self.embedding_dim)
                raise
    ...
    async def embed_batch(
        self, texts: list[str], *, fallback_on_rate_limit: bool = False
    ) -> list[list[float]]:
        if not texts:
            return []
        if not self.provider.is_available or self.provider.name == "local":
        ...
        try:
            ...
            if is_rate_limit_error(exc):
                if fallback_on_rate_limit:
                    logger.warning(
                        "%s batch embed rate limited during import; using zero vectors",
                        self.provider.name,
                    )
                    return [_zero_embedding(self.embedding_dim) for _ in texts]
                raise
        ...
        *,
        ...
        for start in range(0, len(items), batch_size):
            ...
            vectors = await self.embed_batch(
                texts, fallback_on_rate_limit=fallback_on_rate_limit
            )
    ...
    async def embed_movies_for_import(
        ...
        return await self._embed_in_batches(
            pending, on_batch=on_progress, fallback_on_rate_limit=True
        )

    async def apply_zero_user_embeddings_for_import(self, user_id: int) -> int:
        """Fast path: store zero-vector user chunks without calling the embedding API."""
        ...
        rows = result.all()
        zero = _zero_embedding(self.embedding_dim)
        count = 0
